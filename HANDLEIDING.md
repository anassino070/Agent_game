...
5. **Transferwindow** — per cliënt melden zich 0–2 geïnteresseerde clubs. Onderhandelen = de weerstand van de TD naar 0 spelen binnen 5 rondes, en de vólgorde van je zetten is de kern:
   ...
   - **Persoonlijkheid** (verborgen; werkt vooral op kansen, mild op weerstand): IJdel (charme slaagt altijd en iets sterker), Koppig (+5 weerstand, stemming zakt nooit onder Zakelijk), Nerveus (druk +20% slaagkans, maar hij loopt sneller weg), Rekenmeester (feiten +15% kans en 1,25× effect, charme doet niets). "Aftasten" kost twee rondes en onthult het type — die kennis blijft de hele run per club bewaard, ook zichtbaar in het window.
   - **Flow**: twee successen op rij geven +50% effect op je volgende zet; een mislukking reset de reeks.
   - **Combo's** (opeenvolgende successen; elk maximaal één keer per gesprek, het scherm toont de lijst): De Goede Cop (charme → charme → feiten, +6), De Slotklap (charme → feiten → charme → druk, +14), De Boekhouder (feiten → feiten tegen een bekende Rekenmeester, +8), Het Ultimatum (clausule → clausule → druk, +10 — maar je fee is dan al 4% gezakt), De Nerveuze Val (druk → druk tegen een bekende Nerveus, +16 — het hoogste, maar risico-op-risico), Slow Play (clausule → charme → feiten → bluf, +12 — de veilige lange route).
...
De perks vormen een boom van **3 takken × 3 rijen × 3 opties = 27 perks** (Kapitaal, Relaties, Vakwerk; structuur in `Meta.TREE`). Elke rij biedt drie keuzes; een rij ontgrendelt zodra je `Meta.TIER_REQ` (5) niveaus in de rij erboven hebt gekocht — binnen dezelfde tak. Het systeem is bewust een grind: een volledige boom kost **~800.000 punten**, en een verloren run levert typisch 25.000–50.000 op (fees ÷ 2,5 + 2.500 per seizoen; winnen +75.000). Reken op zo'n 20+ runs voor een volle boom — elke run laat iets achter. Op het perkscherm zit ook een **reset-knop** (tweestaps-bevestiging) die alle perks naar 0 zet en álle bestede punten teruggeeft, zodat je vrij kunt herspeccen.
