...
- **Biedingsoorlog** (`bidding_war.gd`, event `overboden`) — **volledig herontworpen naar twee assen.** Elke club biedt een **prijs** (waar jouw fee uit komt) én **voorwaarden** 0–100 (speeltijd, afkoopsom, imagerechten — waar het vertrouwen van je cliënt uit komt), en die twee ruilen tegen elkaar in: `push_price()` verhoogt de prijs maar snoeit in de voorwaarden, `push_terms()` doet het omgekeerde en kost dus jóuw fee. De kernvraag is niet "welke knop werkt" maar **hoeveel van het geluk van je cliënt je verkoopt voor je eigen marge**.

  Waarom de oude versie niet werkte: alles lag open (biedingen, ambitie) en er was maar één leerbare regel (bluf niet twee keer dezelfde club), waardoor de optimale lijn vastlag — bluf de verse club, herhaal, accepteer. Erger nog: vertrouwen speelde er géén enkele rol, terwijl dat overal elders in het spel de motor is.

  Drie **profielen** (`PROFILES`, elk maximaal één keer per veiling zodat de clubs echt verschillen) bepalen hoe duur een duw is: **rijk** (diepe zakken, start met slechte voorwaarden), **ambitieus** (basisplek maar beperkte kas), **gebalanceerd** (middenweg). Elke club heeft zichtbaar **geduld** (2–3 duwen): raakt dat op, dan haakt ze af. Bewust zichtbaar — de spanning moet in de afweging zitten, niet in giswerk. `play_off()` laat de achterblijvers bijtrekken naar de koploper, kost hén geduld maar de koploper niet.

  De uitkomst splitst over beide assen: `Game.complete_transfer()` op de prijs, plus `trust_delta()` = `(voorwaarden − 50) / 5` als apart vertrouwenseffect. Doorgerekend bij een marktwaarde van €1mln: maximaal op prijs duwen bij de rijke club geeft ~**€127k fee met −10 vertrouwen**, maximaal op voorwaarden bij de ambitieuze club ~**€45k fee met +10 vertrouwen**. Bijna 3× het geld voor een vertrouwensklap die je vertrekkans (`leave_chance()`) reëel raakt — precies de afweging die de minigame moet stellen. 5 rondes.
