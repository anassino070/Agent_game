...
**De twee uitkomsten staan NAAST elkaar onder de knop**, niet gestapeld: "Bij succes" linksonder, "Bij mislukking" rechtsonder (`_outcome_columns()` / `_outcome_column()` in `main.gd`). Elke kolom staat daarmee aan dezelfde kant als de kleur op de kansbalk erboven — groen links, rood rechts — zodat de afweging in één blik te maken is zonder te scrollen. Twee details: beide kolommen krijgen `SIZE_EXPAND_FILL` **ook als ze leeg zijn**, anders schuift de gevulde kolom naar het midden zodra de andere kant geen effecten heeft; en in RTL spiegelt Godot de kindorde van de `HBoxContainer` zelf, dus daar wordt ook de tekstuitlijning meegespiegeld (`lead`/`trail`) om de kolommen tegen de buitenranden te houden.
