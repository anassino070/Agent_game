...
- **Pokerbluf tegen een rivaal** (`poker_bluff.gd`, event `rivaal_poker`, `needs_slot`) — vereenvoudigde pokerinzet om de rechten op een gedeeld toptalent. Jouw handsterkte is bekend, die van de rivaal niet; Meegaan/Verhogen/Bluffen/Passen, 3 rondes of eerder als iemand past. Winnen levert een nieuwe cliënt op (`new_client`) plus de pot.
- **Dobbelen bij de bookmaker** (`dice_bookmaker.gd`, event `bookmaker_dobbelen`) — Yahtzee-lite: 5 dobbelstenen, 2 herkansingen (vasthouden/opnieuw gooien), uitbetaling op een vaste inzet op basis van de eindcombinatie (van "niets" tot vijf gelijke ogen).
- **De boekhoudpuzzel** (`accounting_puzzle.gd`, event `boekhoud_puzzel`) — een 4×4 mini-sudoku (elke rij/kolom/vak bevat 1-4 precies één keer), 3 controlepogingen. Legitiem en risicoloos alternatief voor de fiscale schikking: puur denkwerk, geen kansspel.
- **Anagramjacht** (`anagram_hunt.gd`, event `anagram_jacht`) — drie gehusselde woorden uit een gelekt document; kies het juiste woord uit vier opties. Score bepaalt de beloning (geld/reputatie).
- **Speed-daten op de scoutingbeurs** (`scout_speed_date.gd`, event `scoutingbeurs_speeddate`) — vier scouts, vier talenten, een verborgen 1-op-1 klik; raad koppels binnen 6 pogingen. Meer koppels = meer scoutpunten/reputatie.
- **Mediatraining: Simon Says** (`simon_media.gd`, event `mediatraining_simon`) — klassiek geheugenspel: bekijk een groeiende reeks "veilige reacties", herhaal haar daarna blind. 5 foutloze rondes = volledig getraind (vertrouwen omhoog, schandaalmeter omlaag); één fout beëindigt de sessie zonder schade.
